...
out_file="${1:-${root}/results/minimal-subagent-smoke-dispatch.json}"
status_file="${2:-${root}/results/minimal-subagent-smoke-status.json}"
...
session_key="agent:main:subagent:issue-16-runtime-diagnostic-minimal-smoke-${session_suffix}"
model="openai/gpt-5.5"

fail() {
  printf '[dispatch-minimal-subagent-smoke-readonly] ERROR: %s\n' "$*" >&2
  exit 1
}
...
patch_params="$(jq -nc \
  --arg key "${session_key}" \
  --arg model "${model}" \
  --arg label "Issue 16 runtime diagnostic minimal smoke ${session_suffix}" \
...
task_message='Runtime diagnostic smoke only. Do not inspect or modify repositories, issues, labels, branches, PRs, skills, workflow config, services, VM settings, or cloud resources. Use the bash tool exactly once to run this smallest read-only command: true. Then answer with only SMOKE_OK if the command executed successfully, or SMOKE_FAILED plus the exact error if it failed.'

agent_params="$(jq -nc \
  --arg key "${session_key}" \
  --arg msg "${task_message}" \
  --arg idk "issue-16-runtime-diagnostic-minimal-smoke-${session_suffix}" \
...
tail_text="$(
  if [[ -n "${session_id}" && -f "${state_dir}/agents/main/sessions/${session_id}.jsonl" ]]; then
    tail -n 160 "${state_dir}/agents/main/sessions/${session_id}.jsonl"
  else
    printf ''
  fi
)"
