...
out_file="${1:-${workflow_root}/results/developer-dispatch-recovery-after-userns-fix-preflight.json}"

fail() {
  printf '[preflight-issue-16-developer-dispatch-recovery-after-userns-fix] ERROR: %s\n' "$*" >&2
  exit 1
}
...
[[ -f /etc/apparmor.d/openclaw-codex-bwrap ]] || fail "missing narrow AppArmor profile: /etc/apparmor.d/openclaw-codex-bwrap"
grep -qE '^devclaw-svc:' /etc/subuid || fail "missing /etc/subuid mapping for devclaw-svc"
grep -qE '^devclaw-svc:' /etc/subgid || fail "missing /etc/subgid mapping for devclaw-svc"
command -v newuidmap >/dev/null 2>&1 || fail "missing newuidmap"
command -v newgidmap >/dev/null 2>&1 || fail "missing newgidmap"
...
if ! jq -e 'any(.labels[].name; . == "Implementation" or . == "Implementing" or . == "Refining")' <<<"${issue_json}" >/dev/null; then
  fail "issue #${issue_id} must be in implementation/recovery state; labels: ${labels}"
fi
if jq -e 'any(.labels[].name; . == "Validating" or . == "Human Review" or . == "Done" or . == "Rejected")' <<<"${issue_json}" >/dev/null; then
  fail "issue #${issue_id} has advanced or become terminal: ${labels}"
fi
...
jq -nc \
  ...
  '{
    checkedAt:$checkedAt,
    repository:{branch:$branch, head:$head, originHead:$originHead, upstream:$upstream, aheadBehind:$aheadBehind, clean:true, synchronized:true, implementationBranch:$implementationBranch, implementationBranchAvailable:true},
    issue:{number:16, state:"open", labels:$labels, bodySha256:$bodyHash, architectureReportIndex:($architectureReportIndex|tonumber), architectureApprovalIndex:($architectureApprovalIndex|tonumber), implementationApprovalIndex:($implementationApprovalIndex|tonumber), noConflictingOpenPr:true},
    runtime:{gatewayTasks:$gatewayTasks, noActiveWorkers:true, sequential:true, heartbeatDisabled:true, autoMergeDisabled:true, autonomousSkillWorkshopDisabled:true, humanReview:true, usernsAppArmorFixValidated:true, runtimeValidation:$runtimeValidation},
    github:{tokenBroker:true},
