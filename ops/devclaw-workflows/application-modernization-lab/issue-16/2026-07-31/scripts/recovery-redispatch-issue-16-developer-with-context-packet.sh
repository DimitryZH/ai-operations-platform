...
session_key="agent:main:subagent:application-modernization-lab-developer-senior-ara-issue-16-context-recovery-${session_suffix}"
session_label="Application Modernization Lab - Developer Ara (Senior Issue 16 Context Recovery ${session_suffix})"
...
context_result="${workflow_root}/results/issue-16-developer-context-packet.json"
...
out_file="${1:-${workflow_root}/results/developer-context-recovery-dispatch-result.json}"
started_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

fail() {
  printf '[recovery-redispatch-issue-16-developer-with-context-packet] ERROR: %s\n' "$*" >&2
  exit 1
}
...
[[ "$(config_get tools.exec.mode)" == "auto" ]] || fail "OpenClaw exec mode must be auto"
[[ "$(config_get tools.exec.strictInlineEval)" == "true" ]] || fail "OpenClaw strictInlineEval must remain true"
...
if ! jq -e 'any(.labels[].name; . == "Implementing")' <<<"${issue_json}" >/dev/null; then
  fail "issue #${issue_id} must be in Implementing for recovery; labels: ${labels_before}"
fi
if jq -e 'any(.labels[].name; . == "Validating" or . == "Human Review" or . == "Done" or . == "Rejected" or . == "To Improve")' <<<"${issue_json}" >/dev/null; then
...
[[ "${old_session_status}" == "done" || "${old_session_status}" == "failed" ]] ||
  fail "expected previous developer session to be done or failed; found ${old_session_status}"
...
labels_payload="$(
  jq -nc --argjson issue "${issue_json}" '
    {
      labels: (
        ([ $issue.labels[].name ] - ["Refining", "To Improve", "Validating", "Human Review", "Done", "Rejected"]) +
        ["Implementing", "notify:openclaw:primary", "owner:Josefina", "developer:senior", "review:human"]
        | unique
      )
    }'
)"
api_patch "https://api.github.com/repos/${repo_full}/issues/${issue_id}" "${labels_payload}" >/dev/null
issue_after_labels="$(api_get "https://api.github.com/repos/${repo_full}/issues/${issue_id}" | jq -r '[.labels[].name] | join(", ")')"
...
jq \
  ...
  '(.projects[] | select(.name==$project).workers.developer.levels.senior[0]) = {
    ...
    name: $name,
    level: "senior",
    recoveryFrom: "blocked developer session",
    recoveredAt: $now
...
Resume the approved Experiment 08B implementation stage after the previous developer session stopped blocked before repository changes.

Authoritative issue context packet:
- Markdown: ${context_md}
- JSON: ${context_json}
- Markdown SHA-256: ${context_md_hash}
- JSON SHA-256: ${context_json_hash}

These files were fetched from GitHub API through the DevClaw GitHub token broker immediately before dispatch. Read the complete context packet first and treat it as the authoritative issue #${issue_id}, accepted issue #14, merged PR #15, architecture report, Human Aspire Architecture Approval revisions, and Human Implementation Approval source. Use direct OpenClaw task tools for any issue comment/report posting. Do not depend on gh issue view for the required issue/comment context.
...
agent_params="$(jq -nc \
  ...
  --arg idk "devclaw-${project}-${issue_id}-developer-senior-context-recovery-${started_at}" \
...
jq -nc \
  ...
  '{
    ...
    role:"developer",
    level:"senior",
    sessionAction:"context-recovery-redispatch",
    oldSessionKey:$oldSessionKey,
    sessionKey:$sessionKey,
    implementationBranch:$branchName,
    contextMarkdown:$contextMarkdown,
    contextJson:$contextJson,
    labelTransition:"Implementing+Refining -> Implementing",
...
jq -nc \
  ...
  --arg labelsAfter "${issue_after_labels}" \
  ...
  '{
    ...
    issue:{number:16, labelTransition:"Implementing+Refining -> Implementing", labelsFinal:$labelsAfter},
