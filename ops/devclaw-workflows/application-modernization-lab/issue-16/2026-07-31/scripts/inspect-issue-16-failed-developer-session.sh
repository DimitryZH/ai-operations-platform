...
out_file="${1:-$(dirname "$0")/../results/failed-developer-session-inspection.json}"

fail() {
  printf '[inspect-issue-16-failed-developer-session] ERROR: %s\n' "$*" >&2
  exit 1
}
...
session_file="$(jq -r '.sessionFile // empty' <<<"${session}")"
session_tail="$(
...
experiment_branches="$(runuser -u devclaw-svc -- git -C "${repo}" branch --list 'experiment-08/*' | sed 's/^[ *]*//')"
remote_experiment_branches="$(runuser -u devclaw-svc -- git -C "${repo}" ls-remote --heads origin 'experiment-08/*' | awk '{print $2}' | sed 's#refs/heads/##')"
...
jq -n \
  ...
  --arg experimentBranches "${experiment_branches}" \
  --arg remoteExperimentBranches "${remote_experiment_branches}" \
  --arg sessionTail "${session_tail}" \
  '{
    checkedAt:$checkedAt,
    issue:{state:$issue.state, labels:[$issue.labels[].name], updatedAt:$issue.updated_at, comments:$issue.comments},
    developerWorker:$worker,
    developerSession:$session,
    repo:{branch:$repoBranch, head:$repoHead, clean:($repoStatus == ""), status:$repoStatus, localExperimentBranches:$experimentBranches, remoteExperimentBranches:$remoteExperimentBranches},
    openPrs:($prs | map({number,title,head:.head.ref,url:.html_url,draft,createdAt:.created_at,updatedAt:.updated_at})),
