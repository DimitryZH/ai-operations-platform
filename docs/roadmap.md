...
| SRE Platform staging readiness and cost bootstrap foundation | The future SRE Platform staging deployment boundary is documented for a separate GCP project, remote state, IAM/operator model, budget guardrails, approval gates, staging components, read-only evidence surfaces, rollback expectations, and a cost-bounded bootstrap plan that treats logging/monitoring as a first-class cost risk. The minimal foundation project, billing boundary, CAD budget alert, protected remote-state bucket, labels, and IAM baseline now exist. | Foundation only. No Terraform plan or apply was performed, no Kubernetes cluster, node pool, GitOps controller, Prometheus instance, Online Boutique workload, controlled traffic, logging/monitoring ingestion resource, Scheduler change, live investigation, GitHub investigation publication, HolmesGPT call, model call, or SRE Platform repository change is claimed. |
...
1. **Create the SRE Platform staging deployment issue**: reuse the separate
   SRE Platform GCP foundation, then define the exact Terraform plan, live
   read-only access model, Kubernetes/GitOps/Prometheus deployment boundary,
   observability cost controls, approval gates, cleanup expectations, and
   evidence contract before any cluster, workload, or ingestion-heavy write.
2. **Complete SRE Platform staging demonstration**: run the approved
   SRE-owned controlled scenario and collect only permitted read-only evidence.
3. **Promote beyond replay fixtures only after staging evidence**: replace or
...
The SRE Platform repository remains read-only context for this workstream; no SRE Platform repository change is claimed by the foundation work.
...
- creating an SRE Platform GCP project or live staging deployment before a
  separate issue defines budget, remote state, read-only preflight, and direct
  approval gates
