...
| SRE Platform staging readiness and cost bootstrap boundary | The future SRE Platform staging deployment boundary is documented for a separate GCP project, remote state, IAM/operator model, budget guardrails, approval gates, staging components, read-only evidence surfaces, rollback expectations, and a cost-bounded bootstrap plan that treats logging/monitoring as a first-class cost risk. | Planning/preflight only. No SRE Platform GCP project has been created, no cloud write or Terraform apply was performed, no cluster was accessed, and no live staging validation is claimed. |
...
1. **Create the SRE Platform staging deployment issue**: select or confirm the
   separate SRE Platform GCP project, budget ceiling, remote-state design,
   read-only preflight, exact approval gates, observability cost controls,
   cleanup expectations, and evidence contract before any cloud or cluster
   write.
