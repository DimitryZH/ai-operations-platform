...
| SRE Platform staging readiness boundary | The future SRE Platform staging deployment boundary is documented for a separate GCP project, remote state, IAM/operator model, budget guardrails, approval gates, staging components, read-only evidence surfaces, and rollback expectations. | Planning/readiness only. No SRE Platform GCP project has been created, no cloud write or Terraform apply was performed, no cluster was accessed, and no live staging validation is claimed. |
...
1. **Create the SRE Platform staging deployment issue**: select the separate
   SRE Platform GCP project boundary, budget ceiling, remote-state design,
   read-only preflight, exact approval gates, cleanup expectations, and
   evidence contract before any cloud or cluster write.
